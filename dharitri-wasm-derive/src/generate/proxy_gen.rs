...
pub fn generate_proxy_endpoint_sig(method: &Method) -> proc_macro2::TokenStream {
    ...
    let result = quote! {
        fn #method_name #generics (
            self,
            #(#arg_decl),*
        ) -> dharitri_wasm::types::ContractCall<Self::Api, <#ret_tok as dharitri_wasm::io::EndpointResult>::DecodeAs>
        #generics_where
    };
    result
}
...
pub fn generate_proxy_deploy_sig(method: &Method) -> proc_macro2::TokenStream {
    ...
    let result = quote! {
        fn #method_name #generics (
            self,
            #(#arg_decl),*
        ) -> dharitri_wasm::types::ContractDeploy<Self::Api>
        #generics_where
    };
    result
}
...
pub fn generate_proxy_endpoint(m: &Method, endpoint_name: String) -> proc_macro2::TokenStream {
    ...
    let mut token_expr = quote! { dharitri_wasm::types::TokenIdentifier::<Self::Api>::moax() };
    let mut nonce_count = 0;
    let mut nonce_expr = quote! { 0u64 };
    let mut payment_count = 0;
    let mut payment_expr = quote! { dharitri_wasm::types::BigUint::<Self::Api>::zero() };
    let mut multi_count = 0;
    let mut multi_expr =
        quote! { dharitri_wasm::types::ManagedVec::<Self::Api, DctTokenPayment<Self::Api>>::new() };
...
pub fn proxy_trait(contract: &ContractTrait) -> proc_macro2::TokenStream {
    ...
    quote! {
        pub trait ProxyTrait:
            dharitri_wasm::contract_base::ProxyObjBase
            + Sized

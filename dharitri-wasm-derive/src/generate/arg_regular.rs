    ...
    match &arg.ty {
        syn::Type::Reference(type_reference) => {
            ...
            if let syn::Type::Slice(slice_type) = &*type_reference.elem {
                ...
                quote! {
                    dharitri_wasm::load_single_arg::<Self::Api, Box<[#slice_elem]>>(self.raw_vm_api(), #arg_index_expr, #arg_name_expr)
                }
            } else {
                ...
                if let syn::Type::Path(syn::TypePath { path, .. }) = referenced_type {
                    if let Some(ident) = path.get_ident() {
                        if *ident == "str" {
                            ...
                            return quote! {
                                dharitri_wasm::load_single_arg::<Self::Api, Box<str>>(self.raw_vm_api(), #arg_index_expr, #arg_name_expr)
                            };
                        }
                    }
                }
                ...
                quote! {
                    dharitri_wasm::load_single_arg::<Self::Api, #referenced_type>(self.raw_vm_api(), #arg_index_expr, #arg_name_expr)
                }
            }
        },
        _ => {
            quote! {
                dharitri_wasm::load_single_arg::<Self::Api, #arg_ty>(self.raw_vm_api(), #arg_index_expr, #arg_name_expr)
            }
        },
    }
}
